...
| Renewal | Recurring annual subscription, billed to the card on file until canceled | Standard subscription renewal — no separate renewal conversation. Retired: the old one-time-charge "we'll reach out before your year is up" language and its backing TODO in `billing.ts` — a subscription (annual or otherwise) is the renewal mechanism, Stripe renews it natively. We email a reminder about 30 days before each annual renewal (renewal notice, below). See `content/legal/terms.md` §7(a). |
| Time to live | Setup takes about a minute. You're live within 7 days. | See "7-day promise" below |
| 7-day promise | If you're not live within 7 days, we'll help you get there | No longer backed by a money-back guarantee (retired below) — the trial itself is the risk reversal for the $4,788/yr subscription, which charges nothing regardless of the 7-day timeline (the $149 setup fee is separate — see the Setup fee row above, charged at signup either way). Do not restore "or you don't pay" wording; it implied a guarantee mechanism that no longer exists. |
...
| Delivery status | Describe what AFRA does structurally (screens applicants, feeds candidates into your existing process). Never state or imply that the self-serve platform currently has real operators running through it live, or that a real candidate has gone end-to-end via a live Instagram DM through a real ManyChat flow. | As of September 2026, the delivery chain is proven only as far as: a curl request to `/api/manychat/webhook` correctly creates/rejects a `Candidate`. No operator has ever connected a real, confirmed ManyChat flow end-to-end (`ChannelConnection.status` has never reached `"connected"` for any real operator). The Sandoitchi proof numbers (58 candidates/3 days; 58% completion/10 qualified in 7 days) are from the original *manual* pilot, true and usable as-is, and are a separate claim from "the current platform is live" — don't let copy blend the two. Structural/mechanism claims ("it runs on the Instagram you already use," "candidates get screened") are fine since that's what the code does when exercised; volume or liveness claims about the current customer base are not, until this changes. |
...
**Welcome** — `sendWelcomeAwaitingSetupEmail()`, sent from `confirmFoundingPayment()`
(`src/lib/activation.ts`) on the verified-webhook path (`checkout.session.completed`). States the $149
setup fee was actually charged today (see the Setup fee claims row above) and the $4,788/yr trial terms
(20 free screened candidates, 60-day cap) for what comes after. This is the only place a web signup's
onboarding call gets booked (`bookingUrl` — `OPERATOR_ONBOARDING_CALL_URL`, `src/lib/constants.ts`) —
mirrors the dashboard's own awaiting-setup banner. `sendWelcomeAssignedEmail()`, the flow-pool-assigned
variant, was deleted along with the pool (never stocked, permanently unreachable — see
`manychatPool.ts`'s removal). Idempotent via `welcomeEmailSentAt`.
...
**Renewal notice** (September 2026) — `sendRenewalNoticeEmail()`, sent by `/api/jobs/run-scheduled-emails`
(`runRenewalNoticeJob`) `RENEWAL_NOTICE_DAYS_BEFORE` (30) days before each automatic annual renewal. Backs
the Terms §7(a) reminder commitment and state auto-renewal notice rules (15-45 days for yearly terms in some
states; confirm with counsel). The renewal date is `Operator.subscriptionRenewsAt`, synced from Stripe's
current period end in `applyStripeStatus`; it's null while trialing or once a cancel is scheduled, so those
operators never get it. Idempotent per renewal via `renewalNoticeSentForRenewsAt` (stores which renewal date
was noticed), so each year's renewal gets exactly one notice. States date, $4,788 amount with the monthly
anchor, card-on-file charge, and how to cancel.

All six repeat the trial/pricing terms verbatim from this file where they mention them. Approved
...
| Claim | ToS clause |
|---|---|
| $149 setup fee, charged at checkout, not refunded by canceling | §5(b), §6, §7(c) |
| First 20 screened candidates free, 60-day cap | §5(c) |
| Cancel any time, no subscription charge during the trial | §5, §6, §7 |
