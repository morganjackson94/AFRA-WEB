...
2 sign-off). **Repriced again, later in August 2026**: the one-time $4,788/yr annual charge (itself an earlier August 2026
repricing) is replaced with a real $399/mo subscription and a genuine free trial — the upfront-cost
objection was the single most consistent piece of prospect feedback, and a subscription is what makes a
trial possible at all (a one-time charge has nothing to "trial"). There are no paying customers on the
old model, so this is a clean cutover, not a migration.

## Canonical claims

| Claim | Approved wording | Notes |
|---|---|---|
| Price | $399/month | Flat, recurring, covers all locations. Standard, ongoing pricing — not an introductory or limited-time rate. |
| The trial | Your first 20 screened candidates are free, for up to 60 days | Whichever comes first. A card is required to start (Stripe's default for subscription-mode Checkout), but nothing is charged until the trial ends. `FREE_CANDIDATE_CAP` / `TRIAL_DAYS_BACKSTOP` in `src/lib/billing.ts`. "Screened" means passed your screening (`Candidate.stage` reaching `"screened"` or beyond) — a candidate who doesn't pass never counts against the free 20. |
| Cancel anytime | You can cancel any time, including during the trial, with nothing owed | Self-serve, from the dashboard (`cancelSubscriptionAction`) — a real Stripe cancellation, not a support-ticket process. |
...
| Per-location reflection (wizard, post location-count) | "Across your {bucket} locations, that's as low as ~${X}/location/month" | Personalized, decided 2026-08-01, recomputed off `MONTHLY_PRICE_CENTS` after the subscription repricing (same formula, new base price). `X` = `perLocationMonthlyDollars()` in `src/lib/qualification.ts`, computed off each location bucket's *upper* bound (honest floor — the true per-location cost for anyone in that bucket is at or below `X`). Suppressed entirely for the 1-2 location bucket, where the framing is weakest. Never fired on the landing page (no location count known there) — wizard step 4 only. |
| Renewal | Recurring monthly subscription, billed to the card on file until canceled | Standard subscription renewal — no separate renewal conversation, no annual-notice framing. Retired: the old one-time-charge "we'll reach out before your year is up" language and its backing TODO in `billing.ts` — a subscription is now the renewal mechanism. See `content/legal/terms.md` §7. |
...
As of the subscription repricing (below), the following are **no longer claims**:

- "$4,788/year" / "billed once" / "per year" / any annual framing — the price is a recurring $399/month
...
plainly that billing has started at $399/month; never a surprise. Idempotent via
...
| $399/month, recurring until canceled | §5, §7 |
| Self-serve cancellation | §7 |
...
- Canva ad creative and Meta campaign primary text — updated separately; match wording to this table,
  ...
  still show older pricing framing ($4,788/yr and, before that, $1,990/"10 founding operators"). Fix
  before any future ad spend — an ad promising one price while the page sells another is the exact
  failure mode this file exists to prevent.
- Pricing strategy itself (that the standing price is $399/month with a 20-candidate/60-day trial) —
